...
import { creator_get } from "../model/creator_get";
...
router.post("/insert", async (req, res) => {
    let creators: creator_get = req.body;
    let pidc : number;
    ...
    let midc : number;
    sql = mysql.format("select mid from movies where name = ?",[creators.moviename])
    ...
    sql = "INSERT INTO `creator`(`midc`, `pidc`) VALUES (?,?)";
  ...
  router.delete("/delete/:person/:movie", async (req, res) => {
    ...
    conn.query("delete from creator where midc = ? and pidc = ?", [midc,pidc], (err, result) => {
        if (err) throw err;
        res
          .status(200)
  ...
  router.delete("/deletebyid/:pid/:mid", (req, res) => {
    ...
    conn.query("delete from creator where midc = ? and pidc = ?", [mid,pid], (err, result) => {
       if (err) throw err;
       res
         .status(200)
         .json({ affected_row: result.affectedRows });
